...
struct RevenueRecordView: View {
                    ...
                    Text(filter.displayName)
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 20)
                                .fill(selectedTimeFilter == filter ? appState.currentTabThemeColors.primaryColor : Color.clear)
    ...
    private var revenueChart: some View {
        Chart(viewModel.revenueRecords) { record in
            ...
            .foregroundStyle(
                LinearGradient(
                    colors: [appState.currentTabThemeColors.primaryColor.opacity(0.6), appState.currentTabThemeColors.primaryColor.opacity(0.1)],
                    startPoint: .top,
                    endPoint: .bottom
                )
            )
    ...
    private var chartLoadingView: some View {
        HStack {
            ...
            VStack(spacing: 12) {
                ProgressView()
                    .progressViewStyle(CircularProgressViewStyle(tint: appState.currentTabThemeColors.primaryColor))
                    .scaleEffect(1.2)
